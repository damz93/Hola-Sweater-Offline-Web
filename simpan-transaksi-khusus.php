<?php
   ...
   //include 'csrf.php';
	...
	$cekdulu= "select * from t_transaksi_khusus_temp where OLEH='$oleh' AND KODE_BARANG='$kode_barang'";
	$prosescek= mysqli_query($koneksi, $cekdulu);
	if (mysqli_num_rows($prosescek)>0) {    
		$qtyz = mysqli_query($koneksi,"SELECT * FROM t_transaksi_khusus_temp where OLEH='$oleh' AND KODE_BARANG='$kode_barang'");
	        	...
	        	$total_stok = (int)$stok2 + $qty;
				$total_harga = (int)$harga * (int)$total_stok;
				//$total_harga2 = ((int)$qty * (int)$satuan) - ((int)$diskon*(int)$qty) ;
				$total_harga2 = (int)$total_stok * (int)$harga;
	        	$queryupd="UPDATE t_transaksi_khusus_temp SET TOTAL=".$total_harga.", QTY=".$total_stok.", TOTAL2=".$total_harga2." WHERE KODE_BARANG='$kode_barang' and OLEH='$oleh'";
	...
	/*	$selectcostum = mysqli_query($koneksi,"select * from t_costum where KODE_COSTUM='$kode_costum'");
		...
		//if (strlen($warna)!=0){			
			//$query = "INSERT into t_transaksi_khusus_temp(KENA,KODE_COSTUM,POTONGAN,COSTUM,KODE_BARANG,KODE_DISKON,JENIS_BARANG,HARGA,HARGA_TAMBAHAN,QTY,TOTAL,TOTAL2,SIZE_,WARNA,KETERANGAN,WAKTU,TGL,OLEH) VALUES('$kena','$kode_costum','$potongan','$ket_costum','$kode_barang','$kode_promo','$jenis_barang','$harga_satuan','$totcost','$qty','$total_harga','$total_harga2','$size','$warna','$keterangan','$waktu_skg2','$tgl','$oleh')";
			$query = "INSERT into t_transaksi_khusus_temp(KENA,KODE_BARANG,KODE_DISKON,JENIS_BARANG,HARGA,HARGA_TAMBAHAN,QTY,TOTAL,TOTAL2,SIZE_,WARNA,KETERANGAN,WAKTU,TGL,OLEH) VALUES('$kena','$kode_barang','$kode_promo','$jenis_barang','$harga_satuan','$totcost','$qty','$total_harga','$total_harga2','$size','$warna','$keterangan','$waktu_skg2','$tgl','$oleh')";
