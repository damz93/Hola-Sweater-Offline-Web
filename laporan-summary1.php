...
<html moznomarginboxes mozdisallowselectionprint>
   ...
   <body>
      ...
      <?php
         ...
         if (isset($_POST['tampil_tanggal3'])) {
             ...
             if ((strcmp($tglnya, $tglakhir) == 0) OR empty($tglakhir)) {                 
                 ...
                 $data_barang = mysqli_query($koneksi, "SELECT t_transaksi.KODE_TRANSAKSI,t_transaksi.TGL AS TGL_TRX,SUM(IF((t_transaksi.JENIS_BARANG) <> 'COSTUM' AND t_transaksi.JENIS_BARANG<>'Ongkir' AND t_transaksi.JENIS_BARANG<>'Custom Sablon DTF Only' AND t_transaksi.JENIS_BARANG<>'Totebag', QTY, 0)) AS PCS,SUM(IF((t_transaksi.JENIS_BARANG)='Costum' OR t_transaksi.JENIS_BARANG='Costum Sablon DTF', QTY,0)) AS Costum, IFNULL(p.PENGELUARAN, 0) AS PENGELUARAN,SUM(IF((t_transaksi.PAYMENT)='TRANSFER', TOTAL2,0))AS PEMBAYARAN_TRANSFER, SUM(IF((t_transaksi.PAYMENT)='CASH', TOTAL2,0))AS PEMBAYARAN_CASH,SUM(IF((t_transaksi.PAYMENT)='EDC', TOTAL2,0))AS PEMBAYARAN_EDC,SUM(IF((t_transaksi.PAYMENT)='TRANSFER' AND t_transaksi.JENIS_BARANG <> 'COSTUM' AND t_transaksi.JENIS_BARANG<>'Ongkir' AND t_transaksi.JENIS_BARANG<>'Custom Sablon DTF Only' AND t_transaksi.JENIS_BARANG<>'Totebag', POTONGAN,0)/QTY_DISKON) AS POTONGAN_TRANSFER1,
                      ...
                      FROM t_transaksi 
                      LEFT JOIN (select SUM(NOMINAL) AS PENGELUARAN, TGL FROM t_pengeluaran GROUP BY TGL )p ON (t_transaksi.TGL = p.TGL) WHERE t_transaksi.TGL='" . $tglnya . "' GROUP BY t_transaksi.TGL ORDER BY t_transaksi.TGL ASC");
      ...
      <h5 hidden class="right"><b><?php
         echo $nama;
         ?></b></h5>
      <h6 hidden class="right"><b><?php
         echo $jumtrans;
         ?></b></h6>
      <h6 hidden class="right"><i><?php
         echo "[" . $waktu_skg . "-" . $jam . "]";
         ?></i></h6>
