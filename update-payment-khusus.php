...
session_start();	
	if($_SESSION['status']!="login"){
		...
		$query="UPDATE t_transaksi_khusus set PAYMENT='$paym' WHERE KODE_TRANSAKSI='$koder'";
			if (mysqli_query($koneksi, $query)) {
				//echo "<script>alert('".$pesan."');window.location.href='form-transaksi-2.php';</script>";		
				echo "<script>alert('".$pesan."');window.location.href='javascript:history.go(-1)';</script>";	
			} else {
