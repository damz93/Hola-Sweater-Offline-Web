...
<html moznomarginboxes mozdisallowselectionprint>
   ...
   <body>   
      <?php 
		...
		?>
      <u>
         <h1 class="center">LAPORAN TRANSAKSI KHUSUS</h1>
      </u>
     ...
     <?php
		...
		if (isset($_POST['tampil_semua22'])){			
			...
			$data_barang = mysqli_query($koneksi,"SELECT * from t_transaksi_khusus ORDER BY KODE_TRANSAKSI ASC");
			$data_barang2 = mysqli_query($koneksi,"SELECT * from t_transaksi_khusus WHERE HARGA_TAMBAHAN>0 GROUP BY KODE_TRANSAKSI ORDER BY KODE_TRANSAKSI ASC");
		 }
		...
		{			
			...
			if((strcmp($tglnya,$tglakhir)==0) OR empty($tglakhir)){
				...
				//$data_barang = mysqli_query($koneksi, "SELECT * from t_transaksi_khusus WHERE TGL='".$tglnya."' ORDER BY KODE_TRANSAKSI ASC");
				//$data_barang = mysqli_query($koneksi, "SELECT COSTUMER,TGL,SUM(QTY) AS JUMLAH_QTY,KODE_TRANSAKSI,SUM(TOTAL2) AS TOTAL_BARANG,POTONGAN from t_transaksi_khusus WHERE TGL='".$tglnya."' GROUP BY KODE_TRANSAKSI ORDER BY KODE_TRANSAKSI ASC");
				$data_barang = mysqli_query($koneksi, "SELECT POTONGAN,KODE_TRANSAKSI,TGL from t_transaksi_khusus WHERE TGL='".$tglnya."' GROUP BY KODE_TRANSAKSI ORDER BY KODE_TRANSAKSI ASC");
				$data_barang2 = mysqli_query($koneksi, "SELECT * from t_transaksi_khusus WHERE TGL='".$tglnya."' AND HARGA_TAMBAHAN>0 GROUP BY KODE_TRANSAKSI ORDER BY KODE_TRANSAKSI ASC");
			}
			else{
				$nama = "Laporan Pemasukan(" . formatTanggal($tglnya) ." s.d. ". formatTanggal($tglakhir).")";
				//$data_barang = mysqli_query($koneksi, "SELECT * from t_transaksi_khusus WHERE TGL BETWEEN '".$tgl_awalz."' AND '".$tgl_akhirz."' ORDER BY KODE_TRANSAKSI ASC");
				$data_barang = mysqli_query($koneksi, "SELECT POTONGAN,KODE_TRANSAKSI,TGL from t_transaksi_khusus WHERE TGL BETWEEN '".$tgl_awalz."' AND '".$tgl_akhirz."' GROUP BY KODE_TRANSAKSI ORDER BY KODE_TRANSAKSI ASC");
				$data_barang2 = mysqli_query($koneksi, "SELECT * from t_transaksi_khusus WHERE TGL BETWEEN '".$tgl_awalz."' AND '".$tgl_akhirz."' AND HARGA_TAMBAHAN>0 GROUP BY KODE_TRANSAKSI ORDER BY KODE_TRANSAKSI ASC");
			}
		}
		...
		{	
			...
			if((strcmp($bulawal,$bulakhir)==0) OR empty($bulakhir)){
				...
				//$data_barang = mysqli_query($koneksi, "SELECT * from t_transaksi_khusus WHERE MONTH(TGL)='".$bulanaw."' AND YEAR(TGL)='".$tahunaw."' ORDER BY KODE_TRANSAKSI ASC");
				$data_barang = mysqli_query($koneksi, "SELECT POTONGAN,KODE_TRANSAKSI,TGL from t_transaksi_khusus WHERE MONTH(TGL)='".$bulanaw."' AND YEAR(TGL)='".$tahunaw."' GROUP BY KODE_TRANSAKSI ORDER BY KODE_TRANSAKSI ASC");
				$data_barang2 = mysqli_query($koneksi, "SELECT * from t_transaksi_khusus WHERE MONTH(TGL)='".$bulanaw."' AND HARGA_TAMBAHAN>0 AND YEAR(TGL)='".$tahunaw."' GROUP BY KODE_TRANSAKSI ORDER BY KODE_TRANSAKSI ASC");	
			}
			else{
				$nama = "Laporan Pemasukan Bulanan(" . $bulawal." s.d. ". $bulakhir.")";
				//$data_barang = mysqli_query($koneksi, "SELECT * from t_transaksi_khusus WHERE TGL between '".$tgl_awal."' AND '".$tgl_akhir."' ORDER BY KODE_TRANSAKSI ASC");				
				$data_barang = mysqli_query($koneksi, "SELECT POTONGAN,KODE_TRANSAKSI,TGL from t_transaksi_khusus WHERE TGL between '".$tgl_awal."' AND '".$tgl_akhir."' GROUP BY KODE_TRANSAKSI ORDER BY KODE_TRANSAKSI ASC");
				$data_barang2 = mysqli_query($koneksi, "SELECT * from t_transaksi_khusus WHERE TGL between '".$tgl_awal."' AND '".$tgl_akhir."' AND HARGA_TAMBAHAN>0 ORDER BY KODE_TRANSAKSI ASC");
			}	
		}
		...
		{	
			...
			if((strcmp($tahunnya,$tahunnya2)==0) OR empty($tahunnya2)){				
				...
				//$data_barang = mysqli_query($koneksi, "SELECT * from t_transaksi_khusus WHERE YEAR(TGL)='".$tahunnya."' ORDER BY KODE_TRANSAKSI ASC");	
				$data_barang = mysqli_query($koneksi, "SELECT POTONGAN,KODE_TRANSAKSI,TGL from t_transaksi_khusus WHERE YEAR(TGL)='".$tahunnya."' GROUP BY KODE_TRANSAKSI ORDER BY KODE_TRANSAKSI ASC");
				$data_barang2 = mysqli_query($koneksi, "SELECT * from t_transaksi_khusus WHERE YEAR(TGL)='".$tahunnya."' AND HARGA_TAMBAHAN>0 GROUP BY KODE_TRANSAKSI ORDER BY KODE_TRANSAKSI ASC");	
			}
			else{
				$nama = "Laporan Transaksi Tahunan(" . $tahunnya." s.d. ". $tahunnya2.")";
				
				//$data_barang = mysqli_query($koneksi, "SELECT * from t_transaksi_khusus WHERE TGL between '".$tahunawal_nya."' AND '".$tahunakhir_nya."' ORDER BY KODE_TRANSAKSI ASC");	
				$data_barang = mysqli_query($koneksi, "SELECT POTONGAN,KODE_TRANSAKSI,TGL from t_transaksi_khusus WHERE TGL between '".$tahunawal_nya."' AND '".$tahunakhir_nya."' GROUP BY KODE_TRANSAKSI ORDER BY KODE_TRANSAKSI ASC");
				$data_barang2 = mysqli_query($koneksi, "SELECT * from t_transaksi_khusus WHERE TGL between '".$tahunawal_nya."' AND '".$tahunakhir_nya."' AND HARGA_TAMBAHAN>0 ORDER BY KODE_TRANSAKSI ASC");	
			}
         ...
         <?php 
			...
			$total_diskon_seluruh=0;
              while($data = mysqli_fetch_array($data_barang)){				
				...
				$total_harga=0;
				$total_harga_nya=0;
				$data_transaksi = mysqli_query($koneksi, "SELECT * from t_transaksi_khusus WHERE KODE_TRANSAKSI='".$kode_transaksi."'");
