<?php 
	...
	$kod_res = $_POST['kode_resi'];	
	$status = $_POST['status'];	    
	$sql = mysqli_query($koneksi,"select * from t_transaksi_temp WHERE OLEH='".$oleh."'");
		while($data = mysqli_fetch_array($sql)){
			...
			$nots = $data['NOTES'];
			$query="INSERT INTO t_transaksi(KODE_TRANSAKSI,KETERANGAN,OLEH,TGL,WAKTU,QTY,JENIS_COSTUM,OPSI_COSTUM,NOTES,STATUS)VALUES('$kod_res','$keter','$oleh','$tgl','$waktu','$qty','$jencos','$opscos','$nots','$status')";
	        if (mysqli_query($koneksi, $query)) {				
				echo "<script>alert('data tersimpan');window.location.href='input-order.php';</script>";
			} else {
