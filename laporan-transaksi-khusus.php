	...
	<body>
		...
		<table class="table-borderless dda" style="width: 100%" align='center'>
			...
			while($data = mysqli_fetch_array($data_barang)){								
				...
				$data_transaksi = mysqli_query($koneksi, "select KODE_TRANSAKSI,SUM(QTY) AS PCS from t_transaksi_khusus where KENA='YA' AND KODE_TRANSAKSI='$kode_transaksi' GROUP BY KODE_TRANSAKSI");
				...
				$data_transaksi2 = mysqli_query($koneksi,"SELECT PAYMENT,COSTUMER,SUM(TOTAL2)as TOTAL FROM t_transaksi_khusus WHERE KODE_TRANSAKSI='$kode_transaksi' GROUP BY KODE_TRANSAKSI");
				...
				$potn2=0;
				$diskon  = mysqli_query($koneksi, "select POTONGAN,KODE_TRANSAKSI from t_transaksi_khusus where KODE_TRANSAKSI='$kode_transaksi' GROUP BY KODE_TRANSAKSI");
				...
				*/
				...
				$costum  = mysqli_query($koneksi, "select SUM(QTY) AS COSTUM,JENIS_BARANG from t_transaksi_khusus where (KENA='TIDAK' or JENIS_BARANG='Costum') AND JENIS_BARANG<>'Shooping Bag' AND KODE_TRANSAKSI='$kode_transaksi' GROUP BY KODE_TRANSAKSI");
				...
				*/
				...
				$pembayaranedc  = mysqli_query($koneksi, "select SUM(TOTAL2) AS EDC,POTONGAN from t_transaksi_khusus where PAYMENT='EDC' AND KODE_TRANSAKSI='$kode_transaksi'");
				while($data4= mysqli_fetch_array($pembayaranedc)){
				$tot_edc = $data4['EDC'];							
				$tot_edctamp = "Rp".number_format($tot_edc,0,',','.');
				$potnedc = $data4['POTONGAN'];		
				...
				$pembayarantf  = mysqli_query($koneksi, "select SUM(TOTAL2) AS TRANSFER,POTONGAN from t_transaksi_khusus where PAYMENT='TRANSFER' AND KODE_TRANSAKSI='$kode_transaksi'");
				while($data5= mysqli_fetch_array($pembayarantf)){
					$tot_tf = $data5['TRANSFER'];							
					$potntf = $data5['POTONGAN'];		
				...
				$pembayarancash  = mysqli_query($koneksi, "select SUM(TOTAL2) AS CASH,POTONGAN from t_transaksi_khusus where PAYMENT='CASH'  AND KODE_TRANSAKSI='$kode_transaksi'");
				while($data6= mysqli_fetch_array($pembayarancash)){
				$tot_cash = $data6['CASH'];							
				$tot_cashtamp = "Rp".number_format($tot_cash,0,',','.');	
				$potncash = $data6['POTONGAN'];		
